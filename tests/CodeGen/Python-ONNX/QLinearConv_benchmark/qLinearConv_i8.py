...
def generate_qlinearconv_nchw_model_int8(
    input_name: str,
    output_name: str,
    ...
    """
    Genera un modello ONNX con un singolo nodo QLinearConv in INT8 (signed).

    Convenzioni:
      - input:  [N, C_in, H, W] (int8)
      - pesi:   [C_out, C_in/group, kH, kW] (int8)
    ...
    # Nota: randint high è esclusivo, quindi 128 include 127
    x_zero_point = np.int8(random.randint(-128, 127))

    # Weight parameters
    w_scale = np.float32(np.random.uniform(0.001, 0.1))
    # Per i pesi spesso si usa 0 come zero_point (simmetrico), ma qui lasciamo random valido int8
    w_zero_point = np.int8(random.randint(-128, 127)) 

    # Output parameters
    y_scale = np.float32(np.random.uniform(0.001, 0.1))
    y_zero_point = np.int8(random.randint(-128, 127))

    # Dati dei pesi (int8) e bias (int32)
    # np.random.randint(-128, 128, ...) genera numeri tra -128 e 127
    ...
    initializers = [
        ...
        # IMPORTANTE: TensorProto.INT8 per zero points e dati
        helper.make_tensor(x_zero_point_name, TensorProto.INT8, [], [x_zero_point]),
        helper.make_tensor(w_name,            TensorProto.INT8, weight_shape, w_data.flatten().tolist()),
        helper.make_tensor(w_scale_name,      TensorProto.FLOAT, [], [w_scale]),
        helper.make_tensor(w_zero_point_name, TensorProto.INT8, [], [w_zero_point]),
        helper.make_tensor(y_scale_name,      TensorProto.FLOAT, [], [y_scale]),
        helper.make_tensor(y_zero_point_name, TensorProto.INT8, [], [y_zero_point]),
        helper.make_tensor(bias_name,         TensorProto.INT32, bias_shape, bias_data.flatten().tolist()),
    ]

    # value info I/O (INT8)
    input_info = helper.make_tensor_value_info(input_name, TensorProto.INT8, input_shape)
    output_info = helper.make_tensor_value_info(output_name, TensorProto.INT8, output_shape)
    ...
    graph = helper.make_graph(
        [node],
        name=f"QLinearConv_NCHW_bench_int8_{param_id}",
        inputs=[input_info],
        outputs=[output_info],
    ...
    model = helper.make_model(
        ...
        producer_name="zant_qlinearconv_bench_int8",
        ir_version=6,
    )
