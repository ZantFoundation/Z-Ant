...
pub const ZantModules = struct {
    ...
    TensorToImage_mod: *std.Build.Module,
    ...
    pub fn init(b: *std.Build, zantStepOptions: ZantStepOptions) !ZantModules {
        ...
        // --- TensorToImage module (GASF/GADF/MTF encoders; no deps) ---
        const TensorToImage_mod = b.createModule(.{ .root_source_file = b.path("src/TensorToImage/mod.zig") });
        ...
        return ZantModules{
            ...
            .TensorToImage_mod = TensorToImage_mod,
        };
    }
};
