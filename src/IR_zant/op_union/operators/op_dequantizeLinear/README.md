...
- `op_dequantizeLinear.zig` — IR operator struct: parses the ONNX node, performs shape inference, and emits the codegen call via `write_op`.
- `zant_dequantizeLinear.zig` — runtime math implementation (lean and standard variants) called by generated code.
