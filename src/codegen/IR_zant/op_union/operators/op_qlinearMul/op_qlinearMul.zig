...
pub const QLinearMul = struct {
    ...
    pub fn write_op(op: *const QLinearMul, writer: *std.Io.Writer) !void {
        ...
        switch (op.input_A.ty) {
            .u8 => {
                try writer.print("    tensMath.qlinear_mul_lean(u8, f32, u8, {s}, {s}, {s}, {s}, {s}, {s}, {s}, {s}, {s}) catch return {d};\n", .{
                    tensor_A_string,
            ...
            .i8 => {
                try writer.print("    tensMath.qlinear_mul_lean(i8, f32, i8, {s}, {s}, {s}, {s}, {s}, {s}, {s}, {s}, {s}) catch return {d};\n", .{
                    tensor_A_string,
                    tensor_A_scale_string,
                    tensor_A_zero_point_string,
                    tensor_B_string,
