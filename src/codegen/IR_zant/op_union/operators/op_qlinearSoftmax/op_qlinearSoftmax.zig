...
pub const QLinearSoftmax = struct {
    ...
    pub fn write_op(op: *const QLinearSoftmax, writer: *std.Io.Writer) !void {
        ...
        switch (op.input_x.ty) {
            .u8 => {
                try writer.print("    tensMath.qlinear_softmax_lean(u8, f32, u8, {s}, {s}, {s}, {s}, {s}, {s}, {d}) catch return {d};\n", .{
                    tensor_x_string,
            ...
            .i8 => {
                try writer.print("    tensMath.qlinear_softmax_lean(i8, f32, i8, {s}, {s}, {s}, {s}, {s}, {s}, {d}) catch return {d};\n", .{
                    tensor_x_string,
                    tensor_x_scale_string,
                    tensor_x_zero_point_string,
                    tensor_y_scale_string,
