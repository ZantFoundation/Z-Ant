...
- how to use `compare_static_codegen_flags.py` to compare the static memory allocation plans generated with different heuristic flags (see `docs/static_memory_planning.md` for more details).
...
1. **Generate the Input Data:**
   ...
   The input for the script is a JSON object that is generated by the Zig build process. When you run `zig build lib-gen -Ddynamic=false -Dstatic_planning=enabled`, the static memory planning information is written to:
...
```sh
python scripts/compare_static_codegen_flags.py --model=<model> -- -Dsome_flag=value -Danother_flag=value
```
